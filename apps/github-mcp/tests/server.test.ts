...
describe("github-mcp", () => {
  ...
  it("lists the two review tools, with only the advisory one not destructive", async () => {
    ...
    // The same required input on both; the event is the only difference.
  ...
  it("offers no `accusation_follows` and no `held` on either tool (decision 138)", async () => {
    // A session pinned to the old rubric goes on sending both; Zod strips what
    // the schema does not declare, so the call posts and the flags vanish.
    const client = new Client({ name: "test", version: "0.0.0" });
    await client.connect(new StreamableHTTPClientTransport(new URL(`${base}/mcp`)));
    const { tools } = await client.listTools();
    const props = (name: string) =>
      Object.keys((tools.find((t) => t.name === name)?.inputSchema.properties ?? {}) as object);
    expect(props("post_advisory_review")).not.toContain("accusation_follows");
    expect(props("post_blocking_review")).not.toContain("accusation_follows");
    ...
    expect(last?.input.body).not.toContain("· held");
  });
