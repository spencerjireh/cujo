...
describe("checkChannel", () => {
  it("reports the guild and channel name the caller has to store", async () => {
    const result = await checkChannel(fakeDiscord(), { channelId: CHANNEL }, silentLog);
  ...
  it("refuses a channel in another server only when one was asserted", async () => {
    ...
    expect(
      await checkChannel(fakeDiscord(), { channelId: CHANNEL, expectGuildId: "999" }, silentLog),
    ).toEqual({ ok: false, reason: "wrong_guild" });
    expect((await checkChannel(fakeDiscord(), { channelId: CHANNEL }, silentLog)).ok).toBe(true);
  });
  ...
  it("turns a failed permission lookup into a refusal rather than throwing", async () => {
    ...
    await expect(checkChannel(discord, { channelId: CHANNEL }, silentLog)).resolves.toEqual({
      ok: false,
  ...
  it("refuses a role that is not in the server, and allows no role at all", async () => {
    expect(
      await checkChannel(fakeDiscord(), { channelId: CHANNEL, roleId: "999" }, silentLog),
    ).toEqual({
      ok: false,
      reason: "no_such_role",
    });
    expect(
      (await checkChannel(fakeDiscord(), { channelId: CHANNEL, roleId: null }, silentLog)).ok,
    ).toBe(true);
  });
});
