...
/**
 * Who a card names, and where (decision 88, reversing 86's allocation).
 *
 ...
 * "passed" — the last surface still showing a bare glyph (decision 86, on
 ...
 * gone (decision 86), this is what keeps that row from holding one orphaned
 * field.
 */
 ...
 *
 * Display-level only (decision 86): the fold still records one finding per
...
/**
 * The author line, back to the fixed party (decision 88): Cujo's name and its
...
/**
 * A blank row, so the card's sections can breathe (decision 88). A field whose
