 ...
 * `declared` and `deploy_default` are the two layers of the setting; the two
 * floors override either.
 */
export type ModeReason = "declared" | "deploy_default" | "manifest_floor" | "bot_floor";
...
/**
 * Deploy default, then the repository's declaration, then the floors — in
 * that order, and the floors last on purpose. The declaration is text a
