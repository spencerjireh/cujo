...
**Nothing that arrives from Discord may lift a block** (decisions 28, 45, 138).
The commands `discord-interactions.ts` dispatches to hold a
`NotificationStore`, which cannot reach a run's dismissal — keep it that way.
...
GitHub on every command; the pull request's author may not dismiss a block on
their own change (decision 44); and a Bot account may not dismiss at all,
decided from the `user.type` the payload carries (decision 138).
...
sentence anyone in the thread can write lift the block.
