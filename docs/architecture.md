...
| **`sandbox-mcp`** | The other MCP server, and the sandbox itself (decision 113). Five tools — create, exec, write, read, destroy — over one interface with two implementations, chosen by `CUJO_SANDBOX_RUNTIME`, plus a staging route beside the MCP endpoint where `apps/cujo` puts a private repository's trees for the box to copy in (decision 158): `local` is a container on our own host with egress enforced by a gateway outside it and a memory cap that keeps an install from taking the services with it (decision 167), `daytona` is the vendor kept so the move is reversible. The harness provisions nothing; the agent reaches a box only through these five tools, which the harness exposes to the model by name (decision 128). |
