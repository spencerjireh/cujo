...
| **`sandbox-mcp`** | The other MCP server, and the sandbox itself (decision 113). Five tools — create, exec, write, read, destroy — over one interface with two implementations, chosen by `CUJO_SANDBOX_RUNTIME`, plus a staging route beside the MCP endpoint where `apps/cujo` puts a private repository's trees for the box to copy in (decision 158). On a judge run `apps/cujo` calls the same five tools itself, with no model, to run the repository's declared install and tests before the parent's turn (decision 161): `local` is a container on our own host with egress enforced by a gateway outside it and a memory cap that keeps an install from taking the services with it (decision 167), `daytona` is the vendor kept so the move is reversible. The harness provisions nothing; the agent reaches a box only through these five tools, which the harness exposes to the model by name (decision 128). |
...
| **`mapper`** | The repository map (decision 172). A trusted service holding no secret: `apps/cujo` streams a pull request's base and head trees in as tarballs it already fetched, the mapper makes a two-commit git repository of them locally — `detect_changes` wants a history, and an archive carries none — and indexes it with `codebase-memory-mcp`, an MIT C engine compiled from source in our own image at a pinned commit. `/slice` answers what a diff reaches: the changed files, the impacted symbols and their hop distance, paged. Queries go through the engine's graph language rather than its call-tracing tool, which drops the edge a function passed as a value has. It makes no outbound call, so it holds no credential. Optional: with no mapper configured, no review asks one. |
...
| Agent → `sandbox-mcp` → sandbox | MCP on the compose network, then `docker exec` into the box | The PR's code: a public, tokenless `git clone` of the repo checked out at base and head, or, for a private repository, the base and head trees `apps/cujo` staged, piped in through exec stdin under a single-use ticket the brief carries (decision 158). The PR's public metadata (number, SHAs, changed files, title, description). The dependency names from the manifest diff. Cujo's own `sandbox/` sensor code and the commands the subagents run. The run's own id, when the repo is public, so the review can name its evidence page — an id and never a URL, so no hostname crosses (decision 36). For a pinned specifier this pull request adds that this instance detonated within seven days, that earlier run's detonation entry — sandbox-produced data that already crossed the other way, carrying no secret and, for a private source run, no run id (decision 145). | Internal, and nothing in the box. The image, the container runtime and the host paths come from `sandbox-mcp`'s own environment and are never tool inputs, so a caller cannot choose what it runs in. No clone credential exists in the box for a public or a private repository: a private one's trees arrive as bytes, fetched on the trusted side |
...
| `apps/cujo` → `mapper` | HTTP on the compose network | A pull request's base and head trees, as the gzipped tarballs `apps/cujo` fetched from GitHub, under a single-use key; then the changed-file list to ask for a slice. **No token, no clone URL, no hostname** — the same rule the sandbox is held to, for the same reason: the mapper parses code a pull request wrote, and a parser is an attack surface. What comes back is JSON about that code | Internal, and nothing to present. The mapper mints no credential and makes no outbound call; the engine is confined to the trees directory by `CBM_ALLOWED_ROOT` |
...
| `apps/cujo` → GitHub | REST API, then the archive host it redirects to | For a private repository in sandbox mode, the gzipped tar of the base commit and of the head commit (decision 158). Read-only | Installation token minted from the App private key, on the API request only; `fetch` drops it on the cross-origin redirect |
| `apps/cujo` → `sandbox-mcp` → sandbox | MCP on the compose network, then `docker exec` into the box | On a judge run (decision 161): `sandbox_create` with the repository's own `allow_hosts`, then `sandbox_exec` of `sniff.py prepare`, `setup`, the declared `install` and `test` lines as `sh -c <line>` under the sensors, and `sniff.py report`; then `sandbox_destroy` when the turn ends. The argv is the repository's own `.cujo.yml` text at base, the clone URL or the ticket, and Cujo's own sensor commands — what the model sent before, with no model sending it | Internal; no credential, and nothing the pull request did not already choose |
| `apps/cujo` → `sandbox-mcp` | HTTP on the compose network, `PUT /stage/<ticket>/<tree>` | Those two archives, streamed, under a 32-hex ticket minted per run. Bytes of the pull request's own code and nothing else: no token, no URL, no name of the repository. `sandbox-mcp` keeps them on its own tmpfs, single use, swept after thirty minutes; a per-tree cap refuses an oversize archive | Internal; the ticket buys one copy into one box and reads nothing back |
...
3. **Into the sandbox.** On a judge run (decision 161), for a repository whose
   `.cujo.yml` declares its test command, `apps/cujo` does this step and the
   next with no model: it provisions the box, prepares the trees, arms the
   sensors, runs the declared install and tests on both trees, and hands the
   parent the box and the `tests` report in its brief; the parent starts at
   judgment. On a gather run the agent provisions a sandbox through `sandbox-mcp`
   — with the brief's `staged` ticket for a private repository, whose two
   trees `apps/cujo` fetched and staged first (decision 158) — and runs two
   commands. `sniff.py prepare` clones head and adds a worktree at base, or
   adopts the staged trees, and hands
...
- **`sandbox-mcp`** — internal only, reachable by `harness` over the compose
  network and by `cujo` for staging (decision 158), and the one service that
  holds the host's Docker socket (decision 114). Its `/stage` is a tmpfs of
  its own, so a staged tree never rests on the host's disk. At boot it builds the two images a review runs — the sandbox from
